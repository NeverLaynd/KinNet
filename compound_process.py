...
def smiles_to_tgraph(smiles,explicit_H=False, use_chirality=True):
    try:
        mol = Chem.MolFromSmiles(smiles)
    except:
        raise RuntimeError("SMILES cannot been parsed!")
    # Initialize node and edge lists
    anums = mol.GetNumAtoms()
    ...
    for atom in mol.GetAtoms():
        atom_feat = atom_features(atom, explicit_H=explicit_H)
    ...
    try:
        ...
        raise RuntimeError("Atoms nums wrong!")
    ...
    for bond in mol.GetBonds():
        ...
        bond_feat = bond_features(bond, use_chirality=use_chirality)
        edge_feats.append(bond_feat)
        edge_feats.append(bond_feat)

    x = torch.tensor(node_feats, dtype=torch.float32)
    edge_index = torch.tensor(edge_index, dtype=torch.float32).t().contiguous()
